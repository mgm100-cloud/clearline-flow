...
// Calculate QP Start date from quarter end date + QP drift
function calculateQPStartDate(quarterEndDate, qpDrift = -14) {
  if (!quarterEndDate) return null;
  try {
    const qEndDate = new Date(quarterEndDate);
    const qpStartDate = new Date(qEndDate);
    qpStartDate.setDate(qEndDate.getDate() + qpDrift);
    return qpStartDate.toISOString().split('T')[0]; // Return YYYY-MM-DD format
  } catch (e) {
    console.error('Error calculating QP Start date:', e);
    return null;
  }
}

async function fetchLateQPTickers() {
  ...
  for (const row of earningsData || []) {
    ...
    console.log(`Debug: Processing Portfolio ticker: ${ticker}, quarter_end: ${quarterEndDate}, analyst: ${who}, qp_drift: ${qpDrift}`);

    if (!quarterEndDate) {
      console.log(`Debug: Skipping ${ticker} - no quarter end date`);
      continue;
    }
    
    // Calculate QP Start date
    const qpStartDate = calculateQPStartDate(quarterEndDate, qpDrift);
    if (!qpStartDate) {
      console.log(`Debug: Skipping ${ticker} - could not calculate QP Start date`);
      continue;
    }

    // Calculate QP days (days from today to QP Start)
    const qpDays = daysUntilInNY(qpStartDate);
    if (qpDays == null) {
      console.log(`Debug: Skipping ${ticker} - invalid QP days calculation`);
      continue;
    }
    ...
    if (qpDays >= 0 && qpDays <= 14) {
      ...
      if (isLate) {
        console.log(`Debug: Adding late QP ticker: ${ticker}`);
        results.push({
          ticker,
          who,
          quarterEndDate,
        ...
        console.log(`Debug: ${ticker} - not late (has QP call date)`);
      }
    } else {
      console.log(`Debug: Skipping ${ticker} - outside QP date range (${qpDays} days)`);
    }
  }

  console.log(`Debug: Final QP results count: ${results.length}`);
  console.log('Debug: QP Results:', results.map(r => ({ ticker: r.ticker, qpDays: r.qpDays, isLate: !r.qpCallDate })));
...
function buildSummaryEmail(lateItems) {
  if (!lateItems.length) {
    return {
      subject: 'QP Late Check: No late tickers today',
      html: '<p>There are no late QP tickers (0-14 days) today.</p>'
    };
  }
  ...
  const sections = Object.keys(groups)
    ...
    .map((who) => {
      const rows = groups[who]
        .sort((a, b) => (a.qpStartDate < b.qpStartDate ? -1 : 1))
        .map(item => {
          const qpCallBlank = item.qpCallDate ? 'No' : 'Yes';
          const emphasize = item.qpDays < 10;
          ...
          return `
            <tr style="${rowStyle}">
              ...
              <td style=\"padding:6px 8px; border:1px solid #e5e7eb; font-family:Arial, sans-serif;\">${formatDateMMDDYY(item.qpStartDate)}</td>
              <td style=\"padding:6px 8px; border:1px solid #e5e7eb; font-family:Arial, sans-serif;\">${item.qpDays}</td>
              <td style=\"padding:6px 8px; border:1px solid #e5e7eb; font-family:Arial, sans-serif;\">${qpCallBlank}</td>
            </tr>`;
        })
        .join('');
        ...
        <table style=\"border-collapse: collapse;\">
          <thead>
            <tr>
              ...
              <th style=\"padding:6px 8px; border:1px solid #e5e7eb; text-align:left;\">QP Start</th>
              <th style=\"padding:6px 8px; border:1px solid #e5e7eb; text-align:left;\">QP Days</th>
              <th style=\"padding:6px 8px; border:1px solid #e5e7eb; text-align:left;\">QP Call Blank</th>
            </tr>
          </thead>
          <tbody>
            ${rows}
  ...
  const html = `
    <div style=\"font-family: Arial, sans-serif; color: #111;\">
      <h2 style=\"margin:0 0 8px 0;\">Late QP Calls (0-14 days)</h2>
      <p style=\"margin:0 0 12px 0;\">Tickers with blank QP Call dates.</p>
      ${sections}
    </div>`;

  return {
    subject: `QP Late Check: ${lateItems.length} late`,
    html
  };
}
...
function buildAnalystEmail(analyst, lateItems) {
  ...
  return {
    subject: `QP Late Check [${analyst}]: ${lateItems.length} late`,
    html
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const lateItems = await fetchLateQPTickers();
    ...
    console.error('Error in cron-qp-late:', error);
